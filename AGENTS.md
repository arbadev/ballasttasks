...
- **Auth seam**: routes get the caller only through `CurrentUserId` (`api/security.py`), or `StreamingUserId` for a route that streams its body; API tests set both with `sign_in` (`tests/api/conftest.py`), which overrides `get_current_user_id` and `get_streaming_user_id`. No other module touches the app's JWTs, and no response, log line or error ever carries a password, a hash or a token.
