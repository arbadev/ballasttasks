...
@pytest.fixture
async def container(migrated_database_url: str, monkeypatch: pytest.MonkeyPatch,
                    tmp_path: Path) -> AsyncIterator[Container]:
...
async def http(container: Container) -> AsyncIterator[httpx.AsyncClient]:
    ...
    with socket.socket() as sock:
        ...
        server = uvicorn.Server(uvicorn.Config(create_app(container=container),
...
async def test_live_http_attachments_lifecycle_and_failures(http: httpx.AsyncClient,
    ...
    link = await http.post(path + "/links", json={"url": "https://example.com"}, headers=user.headers)
    assert link.status_code == 201
    for url in ("javascript:alert(1)", "data:text/html,x", "file:///etc/passwd", "/relative",
                "https://user:secret@example.com", "https://example.com/" + "x" * 2000):
        rejected = await http.post(path + "/links", json={"url": url}, headers=user.headers)
        assert rejected.status_code == 422
    response = await http.post(path + "/files", files={"file": ("../../report.pdf", PDF, "image/png")},
    ...
    for data, code in ((b"", 422), (b"<svg/>", 415), (b"%PD", 415), (PDF * 100, 413)):
        refused = await http.post(path + "/files", files={"file": ("report.pdf", data)}, headers=user.headers)
        assert refused.status_code == code, refused.text
        assert "detail" in refused.json()
    assert (await http.post(f"/tasks/{uuid.uuid4()}/attachments/files",
                            files={"file": ("a.pdf", PDF)}, headers=user.headers)).status_code == 404
    ...
    assert (await http.delete(path + f"/{link.json()['id']}", headers=user.headers)).status_code == 204
...
async def test_database_failure_after_file_write_compensates_storage(container: Container) -> None:
    async with container.request_scope() as scope:
        user = await scope.register_user.execute(email=f"{uuid.uuid4().hex}@example.com",
                                                full_name="Tester", password="correct horse battery")
        task = await scope.create_task.execute(title="Rollback", created_by=user.id)
    with pytest.raises(IntegrityError):
        async with container.request_scope() as scope:
            await scope.attach_file.execute(task.id, file_name="a.pdf", chunks=pdf_chunks(),
...
async def test_commit_failure_after_file_write_compensates_storage(container: Container) -> None:
    ...
    try:
        with pytest.raises(RuntimeError, match="commit failed"):
            async with container.request_scope() as scope:
                await scope.attach_file.execute(task.id, file_name="a.pdf", chunks=pdf_chunks(), created_by=user.id)
    finally:
