...
export function EmptyProject({ project }: { project: Project }) {
  ...
  const add = () => {
    ...
    if (!text) return;
    setTitle("");
    void commands.create({ title: text });
  };
  ...
  return (
    <section aria-labelledby={`${id}-title`} className="flex animate-bt-in flex-col items-start gap-4 px-6 py-14 max-md:px-4 max-md:py-10">
      ...
      <label className="flex w-[min(460px,100%)] cursor-text items-center gap-3.5 rounded-bt border border-dashed border-line-2 px-3 py-1.5 text-fg-3 transition-[border-color,box-shadow] duration-[160ms] ease-bt focus-within:border-solid focus-within:border-acc focus-within:shadow-[0_0_0_3px_var(--acc-soft)]">
