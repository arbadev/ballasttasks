...
- **Composition roots**: only `apps/api/src/app/bootstrap.py` and `apps/web/src/app/providers.tsx` name concrete classes. No DI framework. A new provider = new adapter + one registry line (`infrastructure/ai/registry.py`, or `infrastructure/identity/registry.py` for single sign-on), no edits to existing code.
...
- **Tasks reference users**: `tasks.created_by` and `tasks.assignee_id` are foreign keys to `users.id` (see "Tasks reference users" in `docs/architecture.md`), so anything that writes a task row, tests and seed data included, stores its users first (`apps/api/tests/postgres.py`: `INSERT_USER`, `user_row`). Task use cases learn about users only through the `UserDirectory` port.
