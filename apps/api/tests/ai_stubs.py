...
def openrouter_error(code: int, message: str = "provider said no") -> dict[str, Any]:
...
def openrouter_over(handler: Handler) -> OpenRouterLanguageModel:
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
    return OpenRouterLanguageModel(client, model=OPENROUTER_MODEL, api_key=TEST_API_KEY)


def gemini_over(handler: Handler) -> GeminiLanguageModel:
    client = httpx.AsyncClient(transport=httpx.MockTransport(handler))
