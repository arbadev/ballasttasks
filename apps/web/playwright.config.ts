...
/**
 * Both servers are reused when something already answers on their port, so two checkouts
 * running the suite on one machine would test each other's app. Give each its own ports.
 */
const appPort = Number(process.env.BT_APP_PORT ?? 47812);
const designPort = Number(process.env.BT_DESIGN_PORT ?? 47811);
