...
async def test_file_roundtrip_and_removal(task_client: httpx.AsyncClient,
                                        request_scopes: RecordingRequestScopes) -> None:
    task = await create_task(task_client)
    path = f"/tasks/{task['key']}/attachments"
    response = await task_client.post(path + "/files", files={"file": ("../../report.pdf", PDF, "text/html")})
    assert response.status_code == 201, response.text
    attached = response.json()
    assert (attached["kind"], attached["name"], attached["content_type"], attached["size_bytes"]) == (
...
async def test_deleting_task_removes_stored_files(task_client: httpx.AsyncClient,
                                                request_scopes: RecordingRequestScopes) -> None:
    task = await create_task(task_client)
    response = await task_client.post(f"/tasks/{task['id']}/attachments/files",
                                     files={"file": ("evil.exe", PNG)})
    assert response.status_code == 201, response.text
...
@pytest.mark.parametrize(("data", "code"), [(b"", 422), (b"<html>evil</html>", 415),
                                           (b"%PD", 415), (PDF * 11, 413)])
async def test_refused_upload_leaves_no_file(task_client: httpx.AsyncClient,
                                           request_scopes: RecordingRequestScopes,
                                           data: bytes, code: int) -> None:
...
async def test_malformed_multipart_is_422(task_client: httpx.AsyncClient, body: bytes) -> None:
    task = await create_task(task_client)
    response = await task_client.post(f"/tasks/{task['id']}/attachments/files", content=body,
...
async def test_oversize_stops_reading_the_network_stream(
    ...
    response = await task_client.post(f"/tasks/{task['id']}/attachments/files", content=network(),
                                     headers={"content-type": "multipart/form-data; boundary=b"})
