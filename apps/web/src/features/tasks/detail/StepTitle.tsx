...
import { useStepRename } from "./useStepRename";
...
export function StepTitle({ taskId, step }: { taskId: string; step: Step }) {
  ...
  const box = useStepRename(taskId, step.id, (text) => track(taskId, commands.renameStep(taskId, step.id, text)));
  const button = useRef<HTMLButtonElement>(null);
  const previous = useRef(false);
  const editing = box.open || box.text !== "" || box.busy;
  useLayoutEffect(() => {
    if (previous.current && !editing && document.activeElement === document.body) button.current?.focus();
    previous.current = editing;
  }, [editing]);
  const type = box.setText;
  const submit = () => {
    if (!validStepTitle(box.text) || box.busy) return;
    box.submit();
  };
  const cancel = box.cancel;
  return <div className="min-w-0 flex-1">
    {editing ? <div data-renaming="" className="flex flex-col gap-2" onKeyDown={(e) => {
      if (e.key === "Escape" && !box.sending) { e.preventDefault(); e.stopPropagation(); cancel(); }
    }}>
      ...
      <div className="flex flex-wrap gap-2">
        <PanelButton variant="secondary" className="px-2 py-1" aria-label="Save step title" disabled={box.busy || !validStepTitle(box.text)} onClick={submit}>Save</PanelButton>
      ...
      {box.failed !== null && <ActionError onRetry={box.retry} onDismiss={box.dismiss}>Could not rename the step. Retry or dismiss the failed save before saving another edit.</ActionError>}
