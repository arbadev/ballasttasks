...
def test_worker_app_is_configured_only_from_settings(minimal_env: pytest.MonkeyPatch) -> None:
    ...
    assert "generate_step_titles" in module.celery_app.tasks
    assert module.celery_app.tasks["generate_step_titles"].track_started is True
    assert module.celery_app.tasks["generate_step_titles"].time_limit == 250
