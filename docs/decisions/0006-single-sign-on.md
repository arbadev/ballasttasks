...
- Every callback failure is the same redirect, `<web callback>?error=sso_failed` (`provider_unavailable` when the provider could not be reached, so the screen can say "try again"; also when the provider refuses this deployment's OAuth client, such as a wrong secret or an unregistered redirect URI, which the adapter reports to the operator with a WARNING). Every exchange failure is the same `401`. Unknown and disabled providers are the same `404`.
...
| Secrets in logs, errors or URLs | Application errors have constant messages. The routes log the error class only, and the Google adapter logs one thing: the OAuth `error` code (a fixed vocabulary, never `error_description` or the body) of a token response that blames this deployment's OAuth client. State, exchange code and binding reach Redis only as SHA-256 digests. The client secret is a `SecretStr` in settings, wrapped in the adapter so it has no `repr`, and sent only in the body of the token request. The only secret that travels in a URL of ours is the one-time exchange code, and the API's access log drops the query string of every `/auth/sso/` request, with or without the path of `SSO__API_PUBLIC_BASE_URL` in front (`api/access_log.py`), so the provider's code and the state are not written there either |
...
- **Set the session as an HttpOnly cookie from the callback instead of issuing the JWT to the web app.** Originally deferred; superseded by [ADR 0009](0009-browser-sessions-and-routes.md). The browser still exchanges the single-use callback code in a POST, now through `/auth/session/sso`, which sets the same JWT as an HttpOnly cookie. Existing bearer exchange and Swagger remain supported.
