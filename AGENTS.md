...
- **Composition roots**: only `apps/api/src/app/bootstrap.py` and `apps/web/src/app/providers.tsx` name concrete classes. No DI framework. A new provider = new adapter + one registry line (`infrastructure/ai/registry.py`, `infrastructure/identity/registry.py` for single sign-on, `infrastructure/storage/registry.py` for file storage), no edits to existing code.
...
- **Steps and activity** ([ADR 0007](docs/decisions/0007-steps-and-activity.md)): a task holds at most 100 steps (`MAX_STEPS_PER_TASK`, checked in `domain/step.py`) at dense positions, and every step writer locks its task with `get_for_update` before it reads them. Record the events specified in ADR 0007 (attaching and removing an attachment: [ADR 0008](docs/decisions/0008-file-storage.md)) through the `ActivityRecorder` port inside the use case and the same unit of work — never from a route or database trigger. Wording lives in `domain/activity_log.py`; silent edits and no-ops log nothing, and one patch may record several changed events.
...
- **Transactions**: repositories never commit. `Container.request_scope()` is the unit of work (one session, commit or rollback in one place); see "Unit of work" in `docs/architecture.md`. Stored files join that transaction only through `FileChanges` (`application/file_changes.py`), which undoes new writes on rollback and defers removals until after commit ([ADR 0008](docs/decisions/0008-file-storage.md)).
- **Auth seam**: routes get the caller only through `CurrentUserId` (`api/security.py`), or `StreamingUserId` for a route that streams its body; API tests set both with `sign_in` (`tests/api/conftest.py`), which overrides `get_current_user_id` and `get_streaming_user_id`. No feature touches JWTs. Browser cookie/CSRF, session lifetime/logout limits, bearer compatibility and URL-owned navigation are in [ADR 0009](docs/decisions/0009-browser-sessions-and-routes.md). Never persist readable browser credentials; never put secrets in logs, URLs or errors.
- **Single sign-on** ([ADR 0006](docs/decisions/0006-single-sign-on.md)): redirect targets come only from the `sso` settings, never from a request; the access token never travels in a URL (one-time code, then `POST /auth/sso/exchange`); accounts are linked only on an email the provider verified. `users.hashed_password` is nullable: a reader must say what "no password" means.
- **Rate limiting**: a new router outside `/health` opts in with `Depends(limit_requests)` (credential routes: `limit_auth_attempts`; a route that streams its body: `limit_streaming_requests`, paired with `StreamingUserId`) and `responses={**TOO_MANY_REQUESTS}` from `api/rate_limit.py`; see "Rate limiting" in `docs/architecture.md`. API tests swap `Container.rate_limiting.limiter`, never Redis.
...
- `apps/api/`: FastAPI service and Celery worker (`src/app/`: `domain`, `application`, `infrastructure`, `api`, `bootstrap.py`, and the entrypoints `main.py`, `worker.py` and `seed_demo.py`).
