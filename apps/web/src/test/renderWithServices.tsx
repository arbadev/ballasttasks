...
import type { Clock } from "@/features/tasks/services/types";
...
export function renderWithServices(ui: ReactElement, options: { tasks?: Task[]; taskService?: FakeTaskService; directoryService?: FakeDirectoryService; clock?: Clock } = {}) {
  ...
  const view = render(
    <Providers taskService={taskService} directoryService={directoryService} stepGenerationService={new FakeStepGenerationService()} clock={options.clock ?? (() => NOW)}>
      {ui}
    </Providers>,
  );
